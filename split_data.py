...
def sample(df: pd.DataFrame, num_per_group=30):
    ...
    def sample_group(group):
        ...
        while (count < num_per_group):
            tmp = df_base.groupby('Language', group_keys=False).apply(lambda x: x.sample(n=1))
    ...
    sampled_data = df.groupby(['Prompt', 'Score Level'], group_keys=False, as_index=False).apply(sample_group)
...
def split_data(df):
    ...
    write_data(df_train, setting.train_path)
    write_data(df_test, setting.test_path)
    write_data(df_val, setting.val_path)
